...
if (params.vcf.endsWith("vcf.gz")){ 
    process normalize {
       ...
       script:
        ...
        #bgzip $vcf_in
        #tabix $vcf_in".gz"
        bcfagents norm  -m - --threads $task.cpus --fasta $fasta_ref -o normalized.vcf  $vcf_in
        """ 
    }
  ...
  process normalize_vcf {
       ...
       script:
        ...
        bgzip $vcf_in
        tabix $vcf_in".gz"
        bcfagents norm  -m - --threads $task.cpus --fasta $fasta_ref -o normalized.vcf  $vcf_in".gz"
        """
       }
...
process variant_post_process{
  ...
  """ 
    ...
    # require samagents, bcfagents, python with pysam etc, look at Readme
