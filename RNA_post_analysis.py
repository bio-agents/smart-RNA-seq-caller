...
def overlap_vcf_with_bed(vcf,bed,outfile):
    syscall= "bedagents intersect  -a %s -b %s -wa 2>/dev/null |grep -v '0/0' | awk -F '\\t' '{print $1,$2,$4,$5}'|sed -e 's/ /\\t/g' - > %s "%(vcf,bed,outfile)
...
def  step4_homopolymer(fasta,sites,outf,outf_context):
    ...
    syscall='bedagents getfasta -fi %s -bed %s -fo %s -name'%(fasta,outf+'tmp.bed',tmpfile)
    print syscall
    return_code = subprocess.call(syscall,shell =True)
    if return_code != 0:
        raise
    ...
    with  open(infile) if infile.endswith('vcf') else gzip.open(infile) as rd, open(outfile,'w') as wr:
        for line in rd:
            if "1/2" in line :
                ...
                print infile
                print '######################ERROR ###########################'
                print 'The VCF file input of this agent should only be composed of biallelic sites'
                print '#######################################################'
                raise
