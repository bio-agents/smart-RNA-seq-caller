    ...
    with  open(args.vcf) if args.vcf.endswith('vcf') else gzip.open(args.vcf) as rd,open(args.outfile,'w') as wr:
        ...
        for line in rd:
            if line.startswith('##'):
                ...
                if len(ff) != 10:
                    print ff
                    print 'Error, this agent only works with single sample VCF'
                    raise
                # generate header
                ...
                # Other more complex genotypes are not processed by this version of the agent
                gt = ff[9].split(':')
        ...
        if args.dna  != 'None':
            ...
            syscall = '''bcfagents isec -n=2 %s %s | awk -F'\t' '{print $1 ":" $2 ":" $3 ":" $4}'  > %s  '''%(args.vcf,args.dna,args.outfile+'.tmp')
